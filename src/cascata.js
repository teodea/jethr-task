...
//
// Il massimale opera poi solo per chi si e' iscritto a forme pensionistiche dopo il
// 31/12/1995 (art. 2 c. 18 L. 335/1995; Circ. INPS 6/2026 par. 6, che lo estende all'1%
// aggiuntivo): per chi era gia' iscritto non esiste, e 9,19% e 1% corrono su tutta la
// retribuzione percepita. `iscrittoAnte1996` e' una deroga, non un input: sotto il
// massimale non cambia una cifra (test/cascata.casi.test.js), e per questo la pagina la
// offre solo sopra (issue #23).
export function contributiDipendente(
  retribuzioneEffettiva,
  costanti,
  { iscrittoAnte1996 = false } = {},
) {
  const c = costanti.contributi
  const baseContributiva = iscrittoAnte1996
    ? retribuzioneEffettiva
    : Math.min(retribuzioneEffettiva, c.massimaleAnnuo)
...
export function calcolaCascata({
  ...
  const contributi = contributiDipendente(retribuzioneEffettiva, costanti, { iscrittoAnte1996 })
